...
async function notifyInstructors(
  ...
  if (offeringId) {
    // Notify instructors assigned to this offering
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <p className="text-xs text-gray-500 mt-1">Required for accessibility.</p>
          </div>
...
export default function AdminAnnouncementsPage() {
  ...
  // ─── Load current admin user + all offerings ─────────────────────
  ...
  const loadOfferings = useCallback(async () => {
    ...
    // Load ALL course offerings (admin sees everything)
    ...
    setOfferings((data ?? []).map((r: any) => ({
      id: r.id,
      label: `${r.courses?.code ?? ''} – ${r.section_name ?? r.id}`,
    })));
  }, []);

  // ─── Load announcements authored by this admin ───────────────────
  ...
  const handleSave = async (publishStatus: Status = 'active') => {
    ...
    // Notify instructors only on new publish
    if (!editingId && resolvedStatus === 'active') {
      await notifyInstructors(supabase, form.offering_id || null, form.title, form.body);
    }
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-2 px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full text-xs font-semibold">For Instructors</span>
        </div>
      ...
      {showForm && (
        <div className="border border-gray-200 rounded-b-xl bg-white p-6 mb-6 shadow-sm">
          ...
          <div className="flex flex-wrap items-end gap-4 mb-4">
            ...
            {/* Offering picker — all offerings for admin */}
          ...
          {!editingId && (
            <div className="mt-3 bg-blue-50 border border-blue-200 rounded p-2.5 text-xs text-blue-700">
              {form.offering_id
                ? 'Instructors assigned to this offering will receive an in-app notification when you publish.'
                : 'All active instructors will receive an in-app notification when you publish.'}
            </div>
          )}
